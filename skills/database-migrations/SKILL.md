...
**一言で**: ゼロダウンタイムDB変更の手順書
**いつ読み込まれるか**: テーブル作成・カラム変更・インデックス追加・データ移行計画時にエージェントが参照する
**チームへの影響**: ClaudeがExpand-Contractパターンや`CREATE INDEX CONCURRENTLY`の活用・バッチデータ移行など本番環境で安全なマイグレーション手順を自動的に選択し、テーブルロックや不可逆な変更によるダウンタイムを防ぐ。
