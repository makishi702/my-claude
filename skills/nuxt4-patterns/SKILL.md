...
**一言で**: Nuxt 4 SSR安全パターン集
**いつ読み込まれるか**: Nuxt 4アプリのビルド・デバッグ時、ハイドレーションエラーやルートルール設定時にエージェントが参照する
**チームへの影響**: ClaudeがNuxt 4固有のSSRとクライアントの差異・データフェッチの使い分け・ルートルール設定を正しく扱うようになり、ハイドレーションミスマッチやパフォーマンス問題を予防する。
