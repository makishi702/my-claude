...
**一言で**: 未知コードベースの構造化分析
**いつ読み込まれるか**: 新規プロジェクト参加時・「このリポジトリを説明して」「CLAUDE.mdを生成して」と依頼された時にエージェントが参照する
**チームへの影響**: Claudeが偵察→アーキテクチャマッピング→規約検出→成果物生成の4フェーズで体系的にコードベースを分析し、スキャン可能なオンボーディングガイドとプロジェクト固有のCLAUDE.mdを自動生成する。
