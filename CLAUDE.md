...
> **既存コードへの適用について**: 既存コードはレガシーな書き方が多く、全てを一度に修正することは現実的でない。
> 以下のルールは**新規に書くコード・編集する箇所**にのみ適用すること。既存コードの違反を見つけても、無関係な修正は行わない。
> ただし「3. シークレットのハードコード禁止」のみ既存コードでも即時対応すること（セキュリティリスクのため）。

1. **テストを先に書く** — TDD 必須。実装の前に必ずテストを書く
2. **console.log / print() をコミットしない** — Python は `logging` モジュール、JS/TS は logger を使う
3. **シークレットのハードコード禁止** — `.env` を使い、コードに直書きしない（**既存コードも対象**）
